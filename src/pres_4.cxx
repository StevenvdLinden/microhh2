    ...
    for (int k=0; k<gd.kmax; k++)
        for (int j=0; j<gd.jmax; j++)
            ...
            for (int i=0; i<gd.imax; i++)
            {
                ...
                p[ijkp]  = (cg0<TF>*(ut[ijk-ii1] + u[ijk-ii1]*dti) + cg1<TF>*(ut[ijk] + u[ijk]*dti) + cg2<TF>*(ut[ijk+ii1] + u[ijk+ii1]*dti) + cg3<TF>*(ut[ijk+ii2] + u[ijk+ii2]*dti)) * cgi<TF>*dxi;
                if (dim3)
                    p[ijkp] += (cg0<TF>*(vt[ijk-jj1] + v[ijk-jj1]*dti) + cg1<TF>*(vt[ijk] + v[ijk]*dti) + cg2<TF>*(vt[ijk+jj1] + v[ijk+jj1]*dti) + cg3<TF>*(vt[ijk+jj2] + v[ijk+jj2]*dti)) * cgi<TF>*dyi;
                        ...
                        const TF* restrict p , const TF* restrict dzhi4)
{
    ...
    for (int j=gd.jstart; j<gd.jend; j++)
        ...
        for (int i=gd.istart; i<gd.iend; i++)
        {
            ...
            ut[ijk] -= (cg0<TF>*p[ijk-ii2] + cg1<TF>*p[ijk-ii1] + cg2<TF>*p[ijk] + cg3<TF>*p[ijk+ii1]) * cgi<TF>*dxi;
            if (dim3)
                vt[ijk] -= (cg0<TF>*p[ijk-jj2] + cg1<TF>*p[ijk-jj1] + cg2<TF>*p[ijk] + cg3<TF>*p[ijk+jj1]) * cgi<TF>*dyi;
        }
        ...
        const TF* restrict u, const TF* restrict v, const TF* restrict w, const TF* restrict dzi4)
{
    ...
    for (int k=gd.kstart; k<gd.kend; k++)
        for (int j=gd.jstart; j<gd.jend; j++)
            ...
            {
                const int ijk = i + j*jj1 + k*kk1;
                TF div = (cg0<TF>*u[ijk-ii1] + cg1<TF>*u[ijk] + cg2<TF>*u[ijk+ii1] + cg3<TF>*u[ijk+ii2]) * cgi<TF>*dxi
                       + (cg0<TF>*v[ijk-jj1] + cg1<TF>*v[ijk] + cg2<TF>*v[ijk+jj1] + cg3<TF>*v[ijk+jj2]) * cgi<TF>*dyi
